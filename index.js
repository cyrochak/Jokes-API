...
app.put("/jokes/:id", (req, res) => {
  ...
  const searchJokeId = jokes.findIndex(joke => joke.id == jokeId);
  jokes[searchJokeId] = newJoke;
  res.json(newJoke);
});
//6. PATCH a joke
app.patch("/jokes/:id", (req, res) =>{
  const jokeId = req.params.id;
  const searchJoke = jokes.find(joke => joke.id == jokeId);
...
app.delete("/jokes/:id", (req,res) => {
  ...
  if (searchJokeId > -1){
    ...
    jokes.splice(searchJokeId, 1);
    res.sendStatus(200);
  } else {
    res.status(404).json({error: `Joke id: ${jokeId} not found.`});
  } 
...
app.delete("/all", (req, res) =>{
  ...
  if (userKey === masterKey){
    ...
    res.status(404).json({error: `masterKey incorrect.`});
  }
});
app.listen(port, () => {
